/**
 * Controller for matches endpoints
 */
 ...
 * @returns {Object} - All matches data
 */
export async function getAllMatchesHandler(req) {
    const { status, limit = 50, offset = 0, minConfidence } = req.query;
    ...
    try {
        ...
        if (minConfidence) {
            filters.confidence = { $gte: parseInt(minConfidence) };
        }
        ...
        req.log.error("Error fetching all matches:", error);
        throw error;
    }
}
